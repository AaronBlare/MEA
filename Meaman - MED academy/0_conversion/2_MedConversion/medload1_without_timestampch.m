...
function [data , time_0 ,Trace_length_points  ] = medload1_without_timestampch( inFilename, DESIRED_CH, DESIRED_TRACES, MEM_LIMIT , var)

%MEDLOAD1ch	Loads data from a medac data file. 
...
chPerCard_0 = readLabeledValue( fh, DAQ_CHANNELS_LABEL );
nDaqCards = length( chPerCard_0 );
...
chPerCard = chPerCard_0(1);
if nDaqCards == 2
nChannels = chPerCard + chPerCard_0(2) ;    
else
...
Trace_length_points = nBlocks * oneShotSize ;
oneTraceSize = fragmentSize * 2 * nChannels;
datasize_Mb = ( nBlocks * oneShotSize ) / 1000000 
if Extended_load
if var.read_one_split_block
    datasize_Mb = ( 8*floor( nBlocks / var.blocks_split_num ) * oneShotSize ) / 1000000 
    sblock_length = floor( nBlocks / var.blocks_split_num ) ;
    data = zeros(( sblock_length ) * oneShotSize, 1 );  % first column for the time stamp
    Trace_length_points = sblock_length * oneShotSize ;
    sblock_i_start = sblock_length * (var.blocks_to_read - 1)+1 ;
    sblock_i_end = sblock_length * (var.blocks_to_read  ) ;
end
else
data = zeros( nBlocks * oneShotSize, 1 );  % first column for the time stamp
end
scaleFactor = amplitude * gain * 10 / 2048 / 50;

Trace_length_sec = (( nBlocks * oneShotSize ) / samplingFreq ) / 1000 ; 
...
%    positions = [];
   ...
   % ---- load piece of channel data
    
   if Extended_load
          if var.read_one_split_block
           for block = 1: sblock_i_end 
               switch nDaqCards  
               case 2
%                  dataBlock = [fread( fd, [ chPerCard_0(1), oneShotSize], 'int16' )', ...
%                        fread( fd, [ chPerCard_0(2), oneShotSize], 'int16' )'];  
%                  whos   dataBlock
                if block >= sblock_i_start && block <= sblock_i_end
                     
                  dataBlock = [fread( fd, [ chPerCard_0(1), oneShotSize], 'int16' )', ...
                       fread( fd, [ chPerCard_0(2), oneShotSize], 'int16' )'];  
                    
                else
%                   fseek( fd,   chPerCard_0(1) * oneShotSize , 'cof' );
%                   fseek( fd,   chPerCard_0(2) * oneShotSize , 'cof' )  ;
                  fseek( fd,   640000 , 'cof' ); 
                end
               otherwise
                 fclose( fd );
               ...
               position = ftell(fd);
%                positions = [ positions ftell(fd)];
               
                if block >= sblock_i_start && block <= sblock_i_end 
                    block_i = block - sblock_i_start + 1 ;
                    data(1 + (block_i - 1) * oneShotSize : oneShotSize + (block_i - 1) * oneShotSize, 1) = ...
                     data(1 + (block_i - 1) * oneShotSize:oneShotSize + (block_i - 1) * oneShotSize, 1) + dataBlock(:, DESIRED_CH);
                end
...
%---- Read loop one channel per func call ----------          
   else
...
%       positions = [ positions ftell(fd)];
      position = ftell(fd);
        ...
        data(1 + (block - 1) * oneShotSize:oneShotSize + (block - 1) * oneShotSize, 1) = ...
         data(1 + (block - 1) * oneShotSize:oneShotSize + (block - 1) * oneShotSize, 1) + dataBlock(:, DESIRED_CH); 
   end
   end
%    figure
%    plot( positions )
...
time_0 = 0 ;
if var.read_one_split_block
    time_0 = oneShotSize * (sblock_length * (var.blocks_to_read - 1)) + 1 ;
    time_0 = time_0 / samplingFreq ;
end
