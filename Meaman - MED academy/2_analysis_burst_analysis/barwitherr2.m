...
function handles = barwitherr2(errors,varargin   )
...
%         handles.h = errorbar(mean(x,1),values(xOrder,col),lowerErrors(xOrder,col), upperErrors(xOrder, col), '.k', 'MarkerSize' , Size );
        handles.h = errorbar(mean(x,1),values(xOrder,col),lowerErrors(xOrder,col), upperErrors(xOrder, col), '.k'  );
        
        errorbar_tick( handles.h , Size );
    end
else
    x = get(get(handles.bar,'children'),'xdata');
%     handles.h = errorbar(mean(x,1),values,errors,'.k' , 'MarkerSize' , Size );
    handles.h = errorbar(mean(x,1),values,errors,'.k'     );
    errorbar_tick( handles.h , Size );
end
