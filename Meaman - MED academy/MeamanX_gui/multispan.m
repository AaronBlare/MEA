...
function varargout = Meamanx(varargin)
...
% Last Modified by GUIDE v2.5 08-Jan-2014 09:24:11
...
PostStimulusResponse.DT_bin = str2num( x{1} );
PostStimulusResponse.PosStim_Interval_start = str2num( a );
PostStimulusResponse.PosStim_Interval_end = str2num( a_to );
PostStimulusResponse.Artefact_channel = str2num( art_c ); 
...
if (get(handles.checkboxPostStim,'Value') == get(handles.checkboxPostStim,'Max'))
    AutoPostStimulusResponse= true ;
else
    AutoPostStimulusResponse = false ;
end
...
PostStimulusResponse.AutoPostStimulusResponse = AutoPostStimulusResponse ;
...
MED_Convert( SIGMATRES , FILE_LIST_PROCESS , ARTEFACTS_ON , sigma_from_sec ,  sigma_to_ms , ...
        Auto_burst_analysis , PostStimulusResponse , Search_Params , Arg_plus  )  
...
c = get(handles.edit25,'String');if (get(handles.checkboxArt,'Value') == get(handles.checkboxArt,'Max'))
   ARTEFACTS_ON = 'y' ;
else
   ARTEFACTS_ON = 'y' ;
end
...
MED_Convert_1ch_artefact( SIGMATRES , 'n' , ARTEFACTS_ON , sigma_from_sec ,  sigma_to_ms ,channel_to_analyze)  
...
% --- Executes on button press in checkboxPostStim.
...
% --- Executes on button press in checkbox6well.
function checkbox6well_Callback(hObject, eventdata, handles)
% hObject    handle to checkbox6well (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of checkbox6well
...
Channel_to_delete = str2num( b );
[filename,pathname] = uigetfile( '*.*' , 'Select file' ) ;

Channel_delete_file_and_save( filename , pathname , Channel_to_delete  )
