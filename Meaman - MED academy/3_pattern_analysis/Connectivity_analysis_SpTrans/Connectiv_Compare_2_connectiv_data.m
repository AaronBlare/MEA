...
function Comp_result = Connectiv_Compare_2_connectiv_data( Connectiv_data1 , Connectiv_data2 , Analysis_data_cell1 , ...
                Analysis_data_cell2 , Analysis_data_cell_field_names , Global_flags )
% Input : Connectiv_data1, Connectiv_data2 , ANALYZED_DATA_1
% ANALYZED_DATA_2 Global_flags
% Output: Comp_result
...
if isfield( Global_flags , 'Show_compare_figure' )
    ...
    Show_compare_figure = true ;
         ...
         %/////////////////////////////////////////////////////////////////
            ...
            Comp_result.Number_of_Connections_file1 = 0 ;
            Comp_result.Number_of_Connections_file2 = 0 ;
            Comp_result.Connections_file1_delays = [] ;
            Comp_result.Connections_file2_delays = [] ;
            Comp_result.Number_of_appear_or_dissappear_Connections = 0 ;
            Comp_result.Number_of_appear_Connections = 0 ;
            Comp_result.Appear_Connections_index_i = [];
            Comp_result.Appear_Connections_index_j = [];
            Comp_result.Number_of_dissappear_Connections = 0 ;
            Comp_result.Dissappear_Connections_index_i =[];
            Comp_result.Dissappear_Connections_index_j =[];
...
%             Comp_result.Number_Spikes2 = sum( ANALYZED_DATA_2.Spike_Rates_each_burst )    ;
            if ~isempty( Analysis_data_cell_field_names)
                Name_index = strmatch( 'Total_spikes_number' , Analysis_data_cell_field_names  ) ;
                if ~isempty( Name_index )
                Comp_result.Number_Spikes = cell2mat( Analysis_data_cell1( Name_index ))  ;
                Comp_result.Number_Spikes2 = cell2mat(  Analysis_data_cell2( Name_index )  )  ;
                end
            end
%             Name_index = strmatch( 'Number_adequate_channels' , Analysis_data_cell_field_names  ) ;
            Comp_result.Number_adequate_channels1 = Connectiv_data1.Number_adequate_channels  ;
%             Comp_result.Number_adequate_channels2 = Analysis_data_cell2( Name_index )  ;
...
%         Connection_delays = []; 
    N = size( Connectiv_data1.Connectiv_matrix_max_M ) ;
    N = N(1) ;
    M_i_diff_common_i = 0 ;
    M_i_diff_unstable_i = 0 ;
    Comp_result.M_i_diff_unstable = [];
    Comp_result.M_i_diff_common = []; 
    Comp_result.tau_max_diff_common = [] ;
    Comp_result.Common_connections_index_i=[  ];
    Comp_result.Common_connections_index_j = []; 
    Comp_result.Dissappear_Connections_index_i=[  ];
    Comp_result.Dissappear_Connections_index_j = [];    
    Comp_result.New_conns_delays=[];
    Comp_result.Dissapeared_conns_delays = [];
    Comp_result.Mean_tau_max_both_files = 0 ;
    
    tau1 = reshape(  Connectiv_data1.Connectiv_matrix_tau_of_max_M , [] ,1 );
    tau2 = reshape(  Connectiv_data2.Connectiv_matrix_tau_of_max_M , [] ,1 );
    tau_all = [ tau1 ; tau2 ] ;
    tau_all( tau_all == 0 ) = [] ;
    Comp_result.Mean_tau_max_both_files = mean( tau_all);
...
%                 Comp_result.M_i_diff_common( M_i ) = -1 ;
               ...
               M1 = Connectiv_data1.Connectiv_matrix_max_M( chi , chj ) ;
               tau1= Connectiv_data1.Connectiv_matrix_tau_of_max_M( chi , chj );
               M2 = Connectiv_data2.Connectiv_matrix_max_M( chi , chj ) ;
               tau2=Connectiv_data2.Connectiv_matrix_tau_of_max_M( chi , chj );
...
%                is_Connection_OK_2 = M2 > 0 ;
               ...
               if Global_flags.Check_Spike_Rates && is_Connection_OK_1                             
                            Enough_spike_both_channels = false; 
                            if Connectiv_data1.Total_Spike_Rates( chi ) >= Global_flags.Connectiv_min_spikes_per_channel && ...
                               Connectiv_data1.Total_Spike_Rates( chj ) >= Global_flags.Connectiv_min_spikes_per_channel  
                                Enough_spike_both_channels = true ;
                            end
                            is_Connection_OK_1  =  Enough_spike_both_channels ; 
               end
                
                if Global_flags.Check_Spike_Rates && is_Connection_OK_2                             
                            Enough_spike_both_channels = false; 
                            if  Connectiv_data2.Total_Spike_Rates( chi ) >= Global_flags.Connectiv_min_spikes_per_channel && ...
                                Connectiv_data2.Total_Spike_Rates( chj ) >= Global_flags.Connectiv_min_spikes_per_channel 
                                Enough_spike_both_channels = true ;
                            end 
                            is_Connection_OK_2 = Enough_spike_both_channels ;
                end 
                
                %-- found connections exists ---
                    ...
                    if is_Connection_exists_1 == 1
                       Comp_result.Connections_file1_delays = [Comp_result.Connections_file1_delays tau1 ] ; 
                    end
                    
                    if is_Connection_exists_2 == 1
                       Comp_result.Connections_file2_delays = [Comp_result.Connections_file2_delays tau2 ] ; 
                    end
%                  
                    % if connections were in both files
                    if  is_Connection_exists_1 == 1 && is_Connection_exists_2 ==1 
                        Comp_result.Number_of_common_Connections = Comp_result.Number_of_common_Connections+1 ;
                        Comp_result.Common_connections_index_i=[Comp_result.Common_connections_index_i chi ];
                        Comp_result.Common_connections_index_j=[Comp_result.Common_connections_index_j chj ];
                        M_i_diff_common_i = M_i_diff_common_i + 1 ;
                        Comp_result.M_i_diff_common( M_i_diff_common_i ) = M2 - M1 ;
                        Comp_result.tau_max_diff_common( M_i_diff_common_i ) = tau2-tau1 ;
%                         if M1-M2 > 0.6
%                             M1 ;
%                             M2 ;
%                         end
                    ...
                    if  is_Connection_exists_1    || ...
                        is_Connection_exists_2    
                        if is_Connection_exists_2   &&  ~ is_Connection_exists_1 % newly appeared connection
                            ...
                            Comp_result.Appear_Connections_index_i=[Comp_result.Appear_Connections_index_i chi];
                            Comp_result.Appear_Connections_index_j=[Comp_result.Appear_Connections_index_j chj];
                            M_i_diff_unstable_i = M_i_diff_unstable_i + 1 ;
                            Comp_result.M_i_diff_unstable( M_i_diff_unstable_i )= M2  ;
%                             new_conn_delay = tau2
                            Comp_result.New_conns_delays = [ Comp_result.New_conns_delays ; tau2 ] ;
                        end
                        
                        if is_Connection_exists_1   &&  ~ is_Connection_exists_2 % dissapeared connection
                            
                            Comp_result.Number_of_appear_or_dissappear_Connections = Comp_result.Number_of_appear_or_dissappear_Connections+1 ; 
                            Comp_result.Number_of_dissappear_Connections=Comp_result.Number_of_dissappear_Connections+1;
                            Comp_result.Dissappear_Connections_index_i =[Comp_result.Dissappear_Connections_index_i chi];
                            Comp_result.Dissappear_Connections_index_j =[Comp_result.Dissappear_Connections_index_j chj];
                            M_i_diff_unstable_i = M_i_diff_unstable_i + 1 ;
                            Comp_result.M_i_diff_unstable( M_i_diff_unstable_i ) = -M1  ;
%                             dissapear_conn_delay = tau1
                            Comp_result.Dissapeared_conns_delays = [ Comp_result.Dissapeared_conns_delays ; tau1 ] ;
                        end
...
%              Comp_result.M_i_diff_common( Comp_result.M_i_diff_common == -1 ) = [] ;
            ...
            Comp_result.Mean_M_difference_common_connections = mean( Comp_result.M_i_diff_common ) ;           
            Comp_result.Mean_tau_max_difference_common_connections = mean( Comp_result.tau_max_diff_common ) ;      
...
%             Comp_result.New_conns_delays = Connectiv_data2.Connectiv_matrix_tau_of_max_M( ...
%                 Comp_result.Appear_Connections_index_i , Comp_result.Appear_Connections_index_j ) ;
%              New_conns_delays = reshape( Comp_result.New_conns_delays , [] , 1 ) ;
%              Comp_result.New_conns_delays = New_conns_delays ;
            Comp_result.New_conns_mean_delay = mean( Comp_result.New_conns_delays ) ;           
            
            
%             Comp_result.Dissapeared_conns_delays = Connectiv_data1.Connectiv_matrix_tau_of_max_M( ...
%                 Comp_result.Dissappear_Connections_index_i ,  Comp_result.Dissappear_Connections_index_j ) ;
%             Comp_result.Dissapeared_conns_delays = reshape( Comp_result.Dissapeared_conns_delays , [] , 1 ) ;
