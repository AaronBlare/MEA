...
function Optimal_parameters = Connectiv_Test_all_Nconn_vs_mintau_minM_from_Cell( ALL_cell , file_number , Global_flags )
...
%                 var.Connectiv_data_index = fi ;
                ...
                var.Strength_THR = Strength_THR;
                var.Connects_min_tau_diff_THR = Connects_min_tau_diff_THR ;
                var.SpBur_thr = Global_flags.Connectiv_min_spikes_per_channel ;
                
%                  Connectiv_get_N_connections_script   
                 [ Number_of_Connections , Connection_delays ] = ...
                     Connectiv_get_N_connections_from_Connectiv( ALL_cell.Connectiv_data( fi  , :)  , Global_flags , var) ;
                %  input 
                % var.default_params 
...
Nconn_plot_2d =  ( Number_of_Connections_all_3d( tau_stable_i_3d(1), strength_stable_i_3d(1) ,  SpBur_stable_i_3d(1) , : ) ) ;
Nconn_plot_2d_optimal_3params = squeeze( Nconn_plot_2d ) ;

Optimal_parameters.tau_stable = tau_stable_3d(1);
Optimal_parameters.strength_stable = strength_stable_3d(1) ;
Optimal_parameters.SpBur_stable = SpBur_stable_3d(1) ; 
Optimal_parameters.Number_of_Connections_vector_optimal = Nconn_plot_2d_optimal_3params ;
Optimal_parameters.RSD = min_SD_3d ;
%--- Figuress-------------------------
...
subplot( Ny , Nx , 1 )
       ...
       legend( min_strength_title{1:length(min_strength_title)} ) 
...
%        plot_2d = squeeze( plot_2d ) ;
  ...
  subplot( Ny , Nx , 8 ) 
       ...
       dat  = ALL_cell.Connectiv_data( 1  ).Total_Spike_Rates ;
       hist( dat )
