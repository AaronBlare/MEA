...
MIN_SPIKES_PER_PATTERN = 4 ;



DELTA_D = 20 ;
Surrogates_type= 'r' ; %'r - random channel
ITERATIONS_JITTER_SURROGATES = 40  ;
NoiseAmp =  200 ;
...
%for Ts = 0 : 10
Global_R_pattern = [] ;
Global_R_surr = zeros(1 , Nb_rand*Nb_rand ) ;
Surr_num = 0 ;
% Global_R_surr=[] ;
Pair_Duration_mean=[];
S = zeros( Nb , Nb) ;
...
%                     Global_R_surr = [ Global_R_surr R ] ;   
    ...
    % Surrogates_type== 'jitterng'
    Global_R_surr=[];
    for iterations=1:ITERATIONS_JITTER_SURROGATES
    for t = 1 : Nb
    for s = 1 : Nb
        if t < s                       
                ch = 0 ;
...
%                 NoiseAmp=0;
                S1 = A( t , : );S2 = A( s , : );
                S1( S1==0 ) = []; S2( S2==0 ) = []; 
                MinA=min(  S1 );MaxA=max(A( t , : ));
                MinA2=min( S2  );MaxA2=max(A( s , : ));
                if isempty( MinA ) MinA=MaxA; end
                    if isempty( MinA2 ) MinA2=MaxA2; end
                    D1=MaxA-MinA;
                    D2=MaxA2-MinA2;
                if PHASE == true 
                    DD1 = avg_dur /dur( s);
                ...
                for k = 1 : N 
                    ...
                    T1 = A( t , k );
                    CN = T1;CN1=0;CN2=0;
                    if T1 -  (NoiseAmp/2) < MinA CN1 =  (NoiseAmp/2) - (T1 - MinA)  ; end 
                    CN = T1 + CN1 ; 
                    if CN +  (NoiseAmp/2) > MaxA CN2 =  (NoiseAmp/2) + CN -MaxA ; end
                    CN= CN - CN2 ;
                    if NoiseAmp > D1
                        T1 = MinA + D1  * 1 *rand()    ;
                    else
                       T1 =  MinA + NoiseAmp*rand() + CN -  NoiseAmp/2 ;                        
                    end   
                     
                    T2 = A( s , k ) ;
                    CN = T2;CN1=0;CN2=0;
                    if T2 -  (NoiseAmp/2) < MinA2 CN1 =  (NoiseAmp/2) - (T2 - MinA2)  ; end 
                    CN = T2 + CN1 ; 
                    if CN +  (NoiseAmp/2) > MaxA2 CN2 =  (NoiseAmp/2) + CN -MaxA2 ; end
                    CN= CN - CN2 ;
                    if NoiseAmp > D2
                        T2 = MinA2 + D2 * 1 *rand() ;
                    else
                       T2 =  MinA2 + NoiseAmp*rand() + CN -  NoiseAmp/2 ;                        
                    end
%                     T1 = D1*rand()    ;
%                      T2 = D2*rand();
...
%  end
%  hist( mean_ch / 20 ) ;
%  xlabel( 'Surrogates_typespike histogram' )
