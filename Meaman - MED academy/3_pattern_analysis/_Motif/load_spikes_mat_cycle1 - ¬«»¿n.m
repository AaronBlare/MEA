...
Relative_step = 0.05 ; 
 x1 = round(floor( mindur / 1 )   *  Relative_step ) * 5 ;
...
if FIRST_SPIKES_ANALYSIS == true
       ...
       ss = find( bursts( t, channel , : ) >= (round(x1 )) ,1 ) ;       
          if isempty( ss ) ~= 1
...
%        burst( t , : ) =  motif( t , : )+burst_start( t );
           ...
           minz =  min( ix  )  ; 
           if isempty( minz ) ~= 1
           DIV = (max( motif( t , : ))-minz ) ;
           else
               DIV = (max( motif( t , : ))-min( motif( t , : ))) ;
           end
%            DIV =1 ;
%            burst( t , : ) =   motif( t , : )/DIV +burst_start( t ) ;
          burst( t , : ) =  motif( t , : )/ (max( motif( t , : ))-min( motif( t , : ))) ;
  end
       plot(  burst( : , :)/1000   , chh(:) ,'.','MarkerEdgeColor' ,[.04 .52 .78] )
