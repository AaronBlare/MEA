function   [ RESULT  ] ...
      = Selective_Channels_Patterns_in_2or3_sets(  Patterns1 , Patterns2 , Patterns3 , ...
      Patterns1_ST , Patterns2_ST , Patterns3_ST , ...
      OVERLAP_TRESHOLD , ...
...
Channel_nonZero_difference_mean_ratio= zeros(  N ,1 );
  ...
  for ch = 1 : N  
      Patterns1_ST_mean( ch )  = mean( Patterns1_ST(  : , ch )); 
      Patterns2_ST_mean( ch )  = mean( Patterns2_ST(  : , ch )); 
      Patterns3_ST_mean( ch )  = mean( Patterns3_ST(  : , ch )); 
  end
...
ttt = 0; 
 ...
 for ch = 1 : N   
        ...
        channel_data1  =  Patterns1_ST( : , ch )' ;  
        channel_data2  =  Patterns2_ST( : , ch )'   ; 
        channel_data3  =  Patterns3_ST( : , ch )'   ; 
      
      channel_data1_zero_values = find(channel_data1==0);
      channel_data1_zero_values = length( channel_data1_zero_values );
      
      channel_data2_zero_values= find(channel_data2==0); 
      channel_data2_zero_values = length( channel_data2_zero_values );
      
      channel_data3_zero_values= find(channel_data3==0); 
      channel_data3_zero_values = length( channel_data3_zero_values );
      
      channel_data1_zero_NONvalues_fraction = (length( channel_data1 ) - channel_data1_zero_values)  / length( channel_data1 ) ;
      channel_data2_zero_NONvalues_fraction =(length( channel_data2 ) - channel_data2_zero_values) / length( channel_data2 ) ;
      channel_data3_zero_NONvalues_fraction =(length( channel_data3 ) - channel_data3_zero_values) / length( channel_data3 ) ;    
      
      Patterns1.Channel_nonZero_vals_percent( ch ) = 100*channel_data1_zero_NONvalues_fraction ;
      Patterns2.Channel_nonZero_vals_percent( ch ) = 100*channel_data2_zero_NONvalues_fraction ;
      Patterns3.Channel_nonZero_vals_percent( ch ) = 100*channel_data3_zero_NONvalues_fraction ;
...
%           if channel_data1_zero_NONvalues_fraction   >= STIM_RESPONSE_BOTH_INPUTS & ...
%                         channel_data2_zero_NONvalues_fraction   >= STIM_RESPONSE_BOTH_INPUTS & ...
%                         channel_data3_zero_NONvalues_fraction >= STIM_RESPONSE_BOTH_INPUTS & ...
...
%           end
          ...
          %------ if channel was active in record 1 and record 2 then use it
          if channel_data1_zero_NONvalues_fraction   >= STIM_RESPONSE_BOTH_INPUTS & ...
                        channel_data2_zero_NONvalues_fraction   >= STIM_RESPONSE_BOTH_INPUTS & ... 
                        Patterns1.Spike_Rates_each_channel_mean(ch)  > 0  & Patterns2.Spike_Rates_each_channel_mean(ch) > 0 & ... 
                        Patterns1.Channels_active( ch ) == 1 & Patterns2.Channels_active( ch ) == 1  
...
%         end
    ...
    ,Zero_values_in_Data1_precent,Zero_values_in_Data2_precent , overlap_val_STABLE_Optim_Thres_precent , is_Stable_response ]= ...
          SelectiveValues( channel_data1 , channel_data2, PVAL_Threshold , OVERLAP_TRES , Count_zero_values ) ;
      ...
      %-- Compare 2nd and 3rd patterns
          [ StatisitcallyDifferent_if1 , LinearyDifferent_if1 , overlap_values_Optim_Thres_precent ,Zero_values_total_precent...
            ,Zero_values_in_Data1_precent,Zero_values_in_Data2_precent , overlap_val_STABLE_Optim_Thres_precent , is_Stable_response ]= ...
                  SelectiveValues( channel_data2 , channel_data3, PVAL_Threshold , OVERLAP_TRES , Count_zero_values ) ;
...
%       OVERLAP_TRESHOLD = 45 ;
     if overlap_values_Optim_Thres_precent <= OVERLAP_TRESHOLD &&  ttt < MAX_FIGURES_data_compare_examples && ...
         length( channel_data1 )>2 && length(channel_data2)>2 && max( [max(channel_data2) max(channel_data1) ] ) >0 
         ttt=ttt+1; 
%          && Zero_values_total_precent < 20
%          Patterns1.Channels_active( ch )
        if Show_Figures
             xxx = max(max(channel_data1),max(channel_data2));
             xxx = 0: xxx/10 :xxx;
             channel_data1' ;
             channel_data2' ;
    %          xxx
               [n,x] = hist(channel_data1 , xxx);
               n=100* n/ length( channel_data1 );
                [n2,x2] = hist(channel_data2 , xxx);
                n2=100* n2/ length( channel_data2 );
                
                f = figure  ;
                set(f, 'name', flags.figure_title ,'numbertitle','off' )
                subplot( 2,1,1);
                bar( [ x' x2' ],[ n' n2' ] )     
%                 plot(   x ,  n ,  x2,  n2   )       
                title( ['Selective electrodes , overlap=' ...
                    num2str( overlap_val_STABLE_Optim_Thres_precent ) ])
                subplot( 2,1,2);
                 hold on

    %             bar( channel_data1 , 'BarWidth',0.6)
    %             bar( channel_data2, 'r','BarWidth',0.3)
                 plot( channel_data1 , '--' , 'MarkerSize'      , 6 ,'Marker'          , '.'  )
                plot( channel_data2, 'r','MarkerSize'      , 6 ,'Marker'          , '.'  )
...
%    Selective_electrodes_lin_div_num
  ...
  Stat_Selective_electrodes_precent2 = 100 *( Stat_selective_electrodes_num2 / total_number_of_active_electrodes2 );
            ...
            %         Patterns2.Spike_Rates_each_channel_mean( ch )  = median(Patterns2.Spike_Rates(  : , ch ));
                ...
                P1_mean = Patterns1_ST_mean(ch) ;
                P2_mean = Patterns2_ST_mean(ch) ;
...
%                                / min( P1_mean , P2_mean ) ) ;
                 ...
                 Channel_difference_mean(ch) =( Patterns2_ST_mean(ch) - ...
                            Patterns1_ST_mean(ch) )  ;
...
%                                / min( P1_mean , P2_mean ) ) ;
            ...
            for ch=1:N
            %     Patterns1_ST_mean( ch )  = median(Patterns1.Spike_Rates(  : , ch ));
                P2_mean = Patterns2_ST_mean(ch) ;
                P3_mean = Patterns3_ST_mean(ch) ;
...
%                                / min( P2_mean , P3_mean ) ) ;
                ...
                Channel_difference_mean2(ch) = ( Patterns3_ST_mean(ch) - ...
                            Patterns2_ST_mean(ch) )  ;
...
%---- all channels comparison ---------------- 
 ...
 n_percent2=100* n_actual/ length( Overlaps_lin_div2 );
  ...
  RESULT.total_number_of_active_electrodes =total_number_of_active_electrodes ;
  RESULT.Stat_selective_electrodes_num = Stat_selective_electrodes_num ;
  RESULT.Stat_Selective_electrodes_precent = Stat_Selective_electrodes_precent ;
  RESULT.Selective_electrodes_lin_div_num= Selective_electrodes_lin_div_num ;
  RESULT.Selective_electrodes_lin_div_precent= Selective_electrodes_lin_div_precent ;
%   TOTAL_RATE.Overlaps_lin_div= Overlaps_lin_div ;
  RESULT.Channels_overlap= Channels_overlap ;
  RESULT.Channels_overlap_only_active =Channels_overlap_only_active; 
  RESULT.Channels_is_selective = Channels_is_selective ;
  RESULT.Channel_is_active = Channel_is_active;
  RESULT.Channel_difference_mean_ratio = Channel_difference_mean_ratio ;
  RESULT.Channel_difference_mean_ratio_only_active = Channel_difference_mean_ratio_only_active ;
  RESULT.Channel_difference_mean = Channel_difference_mean ;
  RESULT.Channel_difference_mean_only_active = Channel_difference_mean_only_active ;
   
 RESULT.Channel_nonZero_difference_mean_ratio  = Channel_nonZero_difference_mean_ratio  ;
...
%   TOTAL_RATE.Channel_difference_std = Channel_difference_std_only_active ;   
  ...
  RESULT.total_number_of_active_electrodes2 =total_number_of_active_electrodes ;
  RESULT.Stat_selective_electrodes_num2 = Stat_selective_electrodes_num ;
  RESULT.Stat_Selective_electrodes_precent2 = Stat_Selective_electrodes_precent ;
  RESULT.Selective_electrodes_lin_div_num2= Selective_electrodes_lin_div_num ;
  RESULT.Selective_electrodes_lin_div_precent2= Selective_electrodes_lin_div_precent ;
%   TOTAL_RATE.Overlaps_lin_div2= Overlaps_lin_div ;
  RESULT.Channels_overlap2= Channels_overlap ;
  RESULT.Channels_overlap2_only_active =Channels_overlap2_only_active; 
  RESULT.Channels_is_selective2 = Channels_is_selective ;
  RESULT.Channel_is_active2 = Channel_is_active;
  RESULT.Channel_difference_mean2_ratio = Channel_difference_mean2_ratio ;
  RESULT.Channel_difference_mean2_ratio_only_active = Channel_difference_mean2_ratio_only_active ;
  RESULT.Channel_difference_mean2 = Channel_difference_mean2 ;
  RESULT.Channel_difference_mean2_only_active = Channel_difference_mean2_only_active ;
  
 RESULT.Channel_nonZero_difference_mean_ratio2  = Channel_nonZero_difference_mean_ratio2  ;
   %//// Use then only active channels characteristics 
...
if Show_Figures 
  ...
  if  Patterns1.Flags(1) == 1   
      ...
      f0 =figure ;
      figure_title= [ flags.figure_title  ];
...
%                 title( 'Data in responses' )
     ...
     subplot(2,1,2)
      ...
      maxval = max( [max(max(Patterns1_ST)) max(max(Patterns1_ST)) max(max(Patterns1_ST)) ] ); 
      delim = Patterns1.N_channels:-1:1 ; 
      delim = 1 : -1/60: 0 ; delim(end)=[];
      delim = delim * maxval ;
      SR = [ Patterns1_ST' delim' Patterns2_ST' delim' Patterns3_ST'  ];
        bb = imagesc(  x , y ,  SR    ) ;
...
%          title( 'Overlap, similar rates %' )
         legend( [ 'Stat different '  flags.figure_details_str ], 'Overlap',4 ,'Box', 'off')
         title( [ 'Compare 3 pattern sets ('  flags.figure_details_str ')' ] ) 
    ...
    srdiff1 = subplot(PlotY,2,3:4);            
        ...
        hold off
            ylabel( flags.figure_details_str )
            title( [ flags.figure_details_str ' difference' ]) 
  ...
  %----- Ctrl1 - Effect - Selective + Overlap channels -------------
      subplot(PlotY,2, 7 :8 );
        ...
        axis( [ min(x)-1 max(x)+1 0 1.1 * max( Channels_overlap2 ) ] ) 
         ...
         legend( 'Stat different data', 'Overlap',4 )
...
% --- plot Ctrl1-ctrl2 vs ctrl2-tet
Mult = 1.5 ;
subplot(Ny,Nx,1);
    hold on
        plot( [ Mult *min(Channel_difference_mean):Mult*max(Channel_difference_mean) ], ...
            [ Mult *min(Channel_difference_mean):Mult*max(Channel_difference_mean) ]  );
        plot( [ Mult *min(Channel_difference_mean):Mult*max(Channel_difference_mean) ], ...
            [  -1* (Mult *min(Channel_difference_mean): Mult*max(Channel_difference_mean)) ]  );        
        plot( Channel_difference_mean ,  Channel_difference_mean2 , '*r' , 'Linewidth' , 2  );
        
        hold off
    xlabel ( 'Ctrl1 Ctrl2')
    ylabel( 'Ctrl2-Tet')
    title('Mean difference of spikerate')
    axis equal
    grid on
...
subplot(Ny,Nx,2);
hold on
    plot( [ Mult *min(Channel_difference_mean_ratio):Mult*max(Channel_difference_mean_ratio) ], ...
        [ Mult *min(Channel_difference_mean_ratio):Mult*max(Channel_difference_mean_ratio) ]  );
  plot( [ Mult *min(Channel_difference_mean_ratio):Mult*max(Channel_difference_mean_ratio) ], ...
        [ -1* ( Mult *min(Channel_difference_mean_ratio): Mult*max(Channel_difference_mean_ratio) ) ]   );    
        plot( Channel_difference_mean_ratio ,  Channel_difference_mean2_ratio , '*r' , 'Linewidth' , 2  );
        hold off
    xlabel ( 'Ctrl1 Ctrl2')
    ylabel( 'Ctrl2-Tet')
    title('Ratio difference of spikerate')
    axis equal
    grid on
 ...
 cr = zeros(N,1); 
 for i = 1 : N
     if Channel_difference_mean(i) ~= 0
     c = abs((abs(Channel_difference_mean2(i))  - abs(Channel_difference_mean(i)))/ Channel_difference_mean(i) ) ;
    cr(i) =   c * sign(Channel_difference_mean2(i))   ;
     else
         cr(i) = (Channel_difference_mean2(i))   ;
     end
 end
%            cr = Channel_difference_mean2  - Channel_difference_mean  ;  
                Plot8x8Data( cr , false  , false  )
                xlabel('Electrode #')
                ylabel('Electrode #')
                title( [ 'Tet-Ctrl2 / Ctrl2-Ctrl1' flags.figure_details_str ]) 
                
 subplot(Ny,Nx,4);  
  cr = zeros(N,1); 
 for i = 1 : N
     if Channel_difference_mean_ratio(i) ~= 0
     c = abs( (abs(Channel_difference_mean2_ratio(i)) - abs(Channel_difference_mean_ratio(i)))/ Channel_difference_mean_ratio(i) )  ;
    cr(i) =   c  * sign(Channel_difference_mean2_ratio(i))   ;
     else
         cr(i) =  Channel_difference_mean2_ratio(i)   ;
         
     end
     
 end
%            cr = Channel_difference_mean2_ratio  - Channel_difference_mean_ratio  ; 
                Plot8x8Data( cr , false  , false  )
                xlabel('Electrode #')
                ylabel('Electrode #')
                title( [ 'Tet-Ctrl2 / Ctrl2-Ctrl1 (ratios)' flags.figure_details_str ])                 
...
if Show_8x8_figures
    f11 = figure;
    smooth = false ;
    set(f11, 'name', flags.figure_title ,'numbertitle','off' )
    Ny = 2 ; Nx =4 ;
 
         minV1 = min( [Overlaps_lin_div   Overlaps_lin_div2] )  ;
     maxV1 = max( [Overlaps_lin_div    Overlaps_lin_div2 ])       ; 
         minV2 = min( [Channel_difference_mean ; Channel_difference_mean2] )  ;
     maxV2 = max( [Channel_difference_mean  ; Channel_difference_mean2 ])       ;      
     minV3 = min( [Channel_nonZero_difference_mean_ratio ; Channel_nonZero_difference_mean_ratio2 ] )  ;
     maxV3 = max( [Channel_nonZero_difference_mean_ratio ; Channel_nonZero_difference_mean_ratio2 ] )       ;  
     
    subplot(Ny,Nx,1);
    Plot8x8Data( Overlaps_lin_div , false  , smooth ,  minV1 , maxV1 )
    xlabel('Electrode #')
    ylabel('Electrode #')
          title( ['Overlaps '  flags.figure_details_str] )
     
    subplot(Ny,Nx,2); 
        Plot8x8Data( Channel_difference_mean , false  , smooth ,  minV2 , maxV2)
        xlabel('Electrode #')
        ylabel('Electrode #')
          title( [ 'Difference ' flags.figure_details_str ])      
          
    subplot(Ny,Nx,3); 
        Plot8x8Data( Channel_nonZero_difference_mean_ratio , false  , smooth ,  minV3 , maxV3 )
        xlabel('Electrode #')
        ylabel('Electrode #')
          title( [ 'Non-zero diff ' flags.figure_details_str ])                
          
    subplot(Ny,Nx,4); 
    Plot8x8Data( Channels_is_selective , false )
    xlabel('Electrode #')
    ylabel('Electrode #')
          title( [ 'Stat. different' flags.figure_details_str ])
%      colormap gray
 
        %---- Ctrl2 - Effect
            subplot(Ny,Nx,5);
            Plot8x8Data( Overlaps_lin_div2 , false , smooth ,  minV1 , maxV1  )
            xlabel('Electrode #')
            ylabel('Electrode #')
                  title( 'Overlaps (Ctrl2-Effect)' )
                  
            subplot(Ny,Nx,6); 
                Plot8x8Data( Channel_difference_mean2 , false  , smooth ,  minV2 , maxV2 )
                xlabel('Electrode #')
                ylabel('Electrode #')
                title( [ 'Difference ' flags.figure_details_str ])    
                
            subplot(Ny,Nx,7); 
                Plot8x8Data( Channel_nonZero_difference_mean_ratio2 , false  , smooth ,  minV3 , maxV3)
                xlabel('Electrode #')
                ylabel('Electrode #')
                title( [ 'Non-zero diff ' flags.figure_details_str ])    
            ...
            xlabel('Electrode #')
            ylabel('Electrode #')
                  title( 'Stat. different (Ctrl2-Effect)' )
...
f2=figure ;
figure_title = [ flags.figure_title ' , Channel=' num2str( flags.EL_SEL_stim_chan_to_extract) ];
...
%                                    flags.Selectivity_figure_title = 'Normal Responses' ;
  ...
  subplot(PlotY, PlotX , PlotX_dx + 1); 
               ...
               legend( 'Ctrl-Effect' , 'Ctrl1-2' )
                    ...
                    xlabel( flags.figure_details_str )   
                   ylabel( 'Count, %') 
  ...
  subplot(PlotY, PlotX , PlotX_dx + 2); 
                 ...
                 SR_thresh_high = mean(Patterns1_ST_mean ) ;
                 [ x , ind ] = find( Patterns1_ST_mean > SR_thresh_high );
...
%                  Channels_zero_res_increase_good = Channels_zero_res_increase( find( abs(Channels_zero_res_increase ) > 0 ) );
    ...
    subplot(PlotY, PlotX , PlotX_dx + PlotX   +2 );               
            ...
            StatDiff = '' ; 
            if length( Channels_zero_res_increase_good ) > 1 && length(Channels_zero_res_increase2_good)>1
