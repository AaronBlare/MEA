% PROGRAM MED_Spike_Sorting
% Does spike sorting with wavelet on all channel files input - XXX_spikes_CHx.mat
% Runs after MED_Convert
...
handles.par.w_post=  15 ;                      %number of post-event data points stored
handles.par.detection = 'both';              %type of threshold
handles.par.stdmin = 6.00;                  %minimum threshold
...
handles.par.sort_fmax = 8000;               %low pass filter for sorting (default 3000)

handles.par.max_spk = 20000;                % max. # of spikes before starting templ. match.
handles.par.template_type = 'ml';       % nn, center, ml, mahal
...
if strcmp(handles.par.features,'pos');      %number of inputs to the clustering for pca
...
% [files , pathname] = uigetfile('*.*','Select file');
% cd(pathname);
%  channel = 1 ;
file_to_cluster = file ;
 % a = load('-ascii', char(file_to_cluster)  );
  file_input = file_to_cluster ;
...
if   nspk > handles.par.min_clus_abs   
    ...
    if length(class1) > handles.par.min_clus; 
        ...
        subplot(3,5,7); 
            hold
             ...
             %plot(spikes(class1(  1 ),:)','r','linewidth',2) 
             %plot(mean(spikes(class1,:),1),'k','linewidth',1)
    ...
    if length(class2) > handles.par.min_clus;
        ...
        subplot(3,5,8); 
            ...
            %plot(spikes(class2( 1  ),:)','r','linewidth',2) 
           % plot(mean(spikes(class2,:),1),'k','linewidth',1)
