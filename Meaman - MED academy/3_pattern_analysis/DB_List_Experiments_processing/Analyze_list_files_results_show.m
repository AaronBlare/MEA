...
if Global_flags.file_number_of_change  > 0
    ...
    all_bar_color = 'b' ;
           ...
           StatDiff_if_1 = EXP_cell.cell_statistics{ ex_n ,  Name_index } ;
           if StatDiff_if_1 == 1 
               StatAsterix = ' (*)' ;
           else
               StatAsterix = Default_StatAsterix ;
           end
           dat = cell2mat( EXP_cell.cell_data( ex_n , :  , Name_index(1) ) ) ;               
...
%                13 Active_channels_number
           ...
           h02 = subplot(Ny,Nx, fi+1 );
               ...
               Name_index = strmatch( 'Number_of_Patterns' , Comp_result_SNames  ) ;
               StatDiff_if_1 = EXP_cell.cell_statistics{ ex_n ,  Name_index(1) } ;
               if StatDiff_if_1 == 1 
                   StatAsterix = ' (*)' ;
               else
                   StatAsterix = Default_StatAsterix ;
...
%                 Active_channels_number  
                meanx = dat( 1 , : ); % Mean_Burst_Duration
                stdx = dat( 2 , : ); % STD_Burst_Duration
...
%                 mean_Super_Durations
%                 Active_channels_number  
                meanx = dat( 3 , : ); % Mean_InterBurstInterval
                stdx = dat( 4 , : ); % STD_InterBurstInterval
...
%                12 mean_Super_Durations
%                13 Active_channels_number  
                meanx = dat( 7 , : ); % Mean_Spike_Rates_each_burst
                stdx = dat( 8 , : ); % STD_Spike_Rates_each_burst
...
%              Comp_result_SNames2{ 3 } = 'Spike_Rates_each_burst' ;
                       ...
                       for file_i = 1 : n_files
                        for hx_i = 1: Global_flags.histogram_bins_number  
                            file_i
                            hx_i
%                             EXP_cell.cell_histograms{ ex_n ,  DataIndex_hist , file_i , 1 , hx_i } = bar_y( hx_i );
                            d2d_hist(file_i , hx_i ) = EXP_cell.cell_histograms{ ex_n ,  DataIndex_hist  , file_i , 1 , hx_i } ;
%                             xout( hx_i ) = EXP_cell.cell_histograms{ ex_n ,  Name_index  , file_i , 2 , hx_i }  ;
                        end
                       end
                       YStep =  EXP_cell.cell_histograms{ ex_n ,  DataIndex_hist  , 1 , 4 , 1 };
...
%                             xout( hx_i ) = EXP_cell.cell_histograms{ ex_n ,  Name_index  , file_i , 2 , hx_i }  ;
  ...
  subplot( Ny , Nx , cfi + 4  ); 
                       ...
                       for file_i = 1 : n_files
                        for hx_i = 1: Global_flags.histogram_bins_number  
%                             EXP_cell.cell_histograms{ ex_n ,  DataIndex_hist , file_i , 1 , hx_i } = bar_y( hx_i );
                            d2d_hist(file_i , hx_i ) = EXP_cell.cell_histograms{ ex_n ,  DataIndex_hist  , file_i , 3 , hx_i } ;
%                             xout( hx_i ) = EXP_cell.cell_histograms{ ex_n ,  Name_index  , file_i , 2 , hx_i }  ;
                        end
                       end
                        YStep =  EXP_cell.cell_histograms{ ex_n ,  DataIndex_hist  , 1 , 4 , 1 };
