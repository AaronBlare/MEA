...
Global_flags.Calc_Spikerate_profile_1ms_bin = false ; % finds spikerate profile 1ms bin
    ...
    Global_flags.Search_Params.Spike_Rate_Signature_max_duration_auto  = true ;
        Global_flags.Search_Params.Spike_Rate_Signature_max_duration = 300 ; % max duration for 1ms profiles to find smooth Tact    

    
GLOBAL_const.med64_fix_32channels_delay = 0 ; %if 100 then if data=med64, take 1-32 channel spikes and minus 100 ms    
    
Burst_Data_Ver = 4.6 ; % 1 - save bursts to matrix (large file), 2 - bursts in cell array - compact file
Burst_analysis_Version = 4.6  ; %% 3 - from 10.11.13 
...
GLOBAL_const.Save_Patterns_1ms_to_file = false ; % if true then save Spike_Rate_Patterns_1ms and Amp_Patterns_1ms to file

Global_flags.Search_Params.Chambers_Separate_analysis_AB = true ;
    Global_flags.Search_Params.Chambers_Burst_duration_AB = 800 ; % duration of new AB burst
    Global_flags.Search_Params.Chamber_A_electrodes =  [ 1 : 20]  ;
    Global_flags.Search_Params.Chamber_B_electrodes =  [ 15 : 60 ] ;
    Global_flags.Search_Params.Chamber_AB_min_delay = 50 ;
     
Global_flags.Show_additional_profiles = true ;
%----------------------------------------------------------------------
...
Global_flags.High_responses_Threshold = 100 ; %-- If some responses exceed this threshold 
                            ...
                            % have High reponses 
Global_flags.autosave_to_DB = true ;  % false
