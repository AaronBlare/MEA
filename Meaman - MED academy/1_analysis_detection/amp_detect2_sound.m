function [spikes,thr,index] = amp_detect2_sound(x,handles)
...
wave = [] ;
wave = zeros( 1 , length( xf ) ) ;
% wave =  xf( T_start_burst : T_end_burst ) ;
 wave =  xf( 1 : end ) ;
m =max( wave) ;
Norm = 1 / m(1) ;
f= 1 ;
% for f= 1:2:4
% sound(wave* Norm , sr * f )
wavwrite(wave,sr,'sound_xxx' );
% end
