...
if Detect_spikes
    ...
    RUNTIME_Detecting_spikes_now = 1  ;
...
if ~ handles.par.threshold_use_defined
    if handles.par.threshold_fromFit_method
        ...
        noise_std_detect= abs(thr_val) ;
          ...
          sThr_from = 1+  handles.par.collect_sigma_from *sr ;
 sThr_To =   handles.par.collect_sigma_to *sr ;
 
 
        if SD == 'y'
            
        
        noise_std_detect = std(x(  sThr_from : sThr_To ) ) ;
        if SIMPLE_RASTER ~= 'y'  noise_std_detect = std(x( sThr_from  : sThr_To )) ;    end
        else
            noise_std_detect = median(abs(x( sThr_from  : sThr_To )))/0.6745;
            if SIMPLE_RASTER ~= 'y'  noise_std_detect = median(abs(x( sThr_from  : sThr_To )))/0.6745; end
        end
...
%     hold on
   ...
   color = 'b' ;%handles.plot_mulatiple_signals_curr_color ; 
%    hold on
end
